'''
EVAR (Exome Variant Analysis Report) for affected sibs

Given three VCF files of 2 or more affected sibs, return shared variants in at
least two sibs under AR , AR_dbHET and AD.
...
Usage :
    ...
    python EVAR_sibs_0.1.py \
    -a sib1.vcf,sib2.vcf  \ #(sperated by ',' and no spaces)
    -f filters.txt \
    -o output.txt \
    -r hierarchy.txt \
    -t tags.txt \
...
class Sibs(parser.Parser):
    def __init__(self,paths,weights_path, filters_path, output_path, tags_path, save):
        ...
        if save == None:
            self.printReportInputInfo('\tAffected sibs analysis [EVA Report]')
    ...
    def get_var_under_models(self):
        ...
        for gene in self.genes_dict.keys():
            ...
            for i in range(0,cln_idx):
                affected = GTs[:,i].tolist()
                Hom_counts = affected.count(2)
                Het_counts = affected.count(1)
                key = tuple(x for x in WTs[0,i]) # only include child weights , row 0                
                
                # for loci with at least two sibs with HOM
                if Hom_counts  >= 2:
                    ids = []            
                    for n, member in enumerate(sorted(self.affectedTSVs.keys())):
                        if affected[n] == 2:
                            record = self.extractUserFields(self.genes_dict[gene]['POSs'][str(sorted_pos[i])][member]['raw'], header)
                            if member not in ids:
                                ids.append(member)
                    
                    record.append(", ".join(ids))
                    record.append(str(Hom_counts))
...
def main():
    ...
    parser.add_option('-a', '--affected',  type='string',help='path to affected sibs VCF files)', action="callback", callback=splitPaths)
    ...
    paths = opts.affected.split(",")
    
    
    if None in paths or len(paths) < 2:
        print paths
    ...
    mySibs = Sibs(paths, weights_path, filters_path, output_path, tags_path, opts.save)
